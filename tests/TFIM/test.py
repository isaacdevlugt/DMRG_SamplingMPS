...
'''
unitary_dict = {k: test_utils.convert_torch_cplx(v) for k, v in unitaries.create_dict().items()}
...
ED_psi = np.loadtxt("true_psi_N=2_J=0.5_h=3.0")
ED_psi = ED_psi[:,0] + 1j*ED_psi[:,1]

num_samples = 10000
DMRG_samples = np.loadtxt("MPS_sampling_algo_N=2_J=0.5_h=3", dtype=int)
...
for i in range(len(bases)):
    ...
    plt.figure() 
    plt.bar(ED_uniques+0.25, ED_counts, color='blue', label="ED samples", align='center', width=0.25)
    plt.bar(ED_uniques, probs, color='black', label="True prob", align='center', width=0.25)
    plt.bar(ALGO_uniques-0.25, ALGO_counts, color='green', label="ALGO samples", align='center', width=0.25)
    plt.title(basis[0]+basis[1])
    plt.xlabel("Basis state index")
    plt.ylabel("Fractional frequency")
    plt.legend()
    plt.xticks([0,1,2,3])
    plt.savefig("Basis={}.png".format(basis[0]+basis[1]), dpi=500, bbox_inches='tight')
